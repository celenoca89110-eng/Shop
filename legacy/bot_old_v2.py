...
class InvitePanelView(discord.ui.View):
    ...
    async def select_config(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if selected == "none":
            ...
            if config_data:
                embed = discord.Embed(
                    title=f"� Configuration : {selected}",
                    color=THEME,
                )
                embed.add_field(name="Client ID", value=f"`{config_data['client_id'] or 'Non défini'}`", inline=True)
                embed.add_field(name="Permissions", value=f"`{config_data['permissions']}`", inline=True)
...
async def update_bot_activity():
    ...
    # Utiliser un set pour éviter de compter les mêmes personnes sur plusieurs serveurs
    ...
    for guild in bot.guilds:
        ...
        for member in guild.members:
            ...
            # Vérifier si l'admin est en ligne et ajouter son ID au set
...
async def check_unauthorized_servers():
    ...
    await bot.wait_until_ready()
    await asyncio.sleep(10)  # Attendre que le bot soit complètement initialisé
...
class CategorySelect(discord.ui.View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, category: str):
        ...
        # Envoyer le premier message de l'utilisateur s'il existe
        if self.first_message:
            await channel.send(
                f"📩 **Premier message de {ticket_subject.mention} :**\n{self.first_message}"
            )
            # Effacer le premier message après utilisation
        ...
        # Envoyer le message de création et l'épingler
        ...
        # Envoyer un message de confirmation dans le MP de l'utilisateur
        try:
...
async def on_message(message: discord.Message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        if uid in tickets_map:
            ...
            # Stocker le premier message de l'utilisateur
            ...
            if now - cooldown.get(uid, 0) > cooldown_sec:
                ...
                await message.channel.send(embed=embed, view=GuildSelect(message.author, message.content))
